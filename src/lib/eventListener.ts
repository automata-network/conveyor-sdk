...
export async function verifyMetaTxnResponse(provider: JsonRpcProvider, response: Response): Promise<Response> {
  ...
  for (const log of logs) {
    let topicToQuery = '0xf624f223d0e1427abaf1ac2d9cf7c8487cad3018f0a93b5dafa867aed96165a3'
    if (log.topics[0] === topicToQuery) {
      const iface = new Interface(['event MetaStatus(address sender, bool success, string error)'])
