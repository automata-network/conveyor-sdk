 ...
 */
async function calculateFeeOnMatic(token: string, tokenDecimals: number, gasFee: BigNumber): Promise<BigNumber> {
...
export default async function getFeePrice(
  ...
  switch (chainId) {
    ...
    case ChainId.MAINNET:
      return calculateFee(chainId, token, tokenDecimals, gasFee, 'eth', nativeTokenDecimals);
    case ChainId.BSC:
      return calculateFee(chainId, token, tokenDecimals, gasFee, 'bnb', nativeTokenDecimals);
    default:
      return calculateFee(chainId, token, tokenDecimals, gasFee, nativeToken!, nativeTokenDecimals);
  }
}